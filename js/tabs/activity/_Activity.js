...
const menuOptions = [
  { text: 'Favorites', icon: 'ios-star-outline' },
  { text: 'Friends', icon: 'ios-people-outline' },
  { text: 'Past Events', icon: 'ios-calendar-outline' },
];
...
class Activity extends Component {
  ...
  static navigationOptions = {
    tabBarIcon: ({tintColor}) => (
      <FAIcon name="calendar" color="white" size={24} />
    ),
  }

  constructor(props) {
    super(props);
    this.searchDataSource = new ListView.DataSource({
          ...
          <SearchBar
            ...
            onFocus={() => {
              if (!this.state.searchMode)
                this.setState({
                  searchResults: this.state.searchResults.cloneWithRows([]),
                });
        ...
        {this.state.searchResults.getRowCount() ? (
          ...
          <View>
            <Text style={{ textAlign: 'center', marginTop: 10, color: '#aaa' }}>
              {this.state.error ? this.state.error.message : 'no matches'}
            </Text>
          </View>
        )}
      </View>
  ...
  _loadMoreResults() {
    this._searchEvent(this.state.input, this.state.page + 1)
      .then(data => {
        this.setState({
          searchResults: this.state.searchResults.cloneWithRows(
            this.state.searchResults._dataBlob.s1.concat(data)
          ),
  ...
  _renderCalenderEvents() {
    return (
      <View style={{ flex: 1, zIndex: 1 }}>
        <ActivityCalendar screenProps={this.props.screenProps}/>
      </View>
    );
  }
  ...
  _openLocationPicker() {
    ...
    this.props.screenProps.rootNavigation.navigate('LocationPicker');
  }
  ...
  _renderCreateEventButton() {
    return (
      <TouchableOpacity
        //style={[profileStyle.blueBtn, { bottom: 15 }]}
        style={[
          profileStyle.blueBtn,
          { bottom: 15, width: 55, height: 55 },
        ]}
        onPress={() => this.props.screenProps.rootNavigation.navigate('CreateActivityScene')}
      >
  ...
  _renderMenu() {
    const { address, placeName } = this.props.searchLocation;
    return (
      <View
        style={{
          position: 'absolute',
      ...
      >
        ...
        >
          <TouchableOpacity onPress={this._openLocationPicker}>
            <View
              style={{
                flexDirection: 'row',
            ...
            >
              ...
              <Text
                style={{ textAlign: 'right', paddingLeft: 5, maxWidth: 200 }}
              >
                {(address && this._modifyAdress(address)) || placeName}
              </Text>
            </View>
          </TouchableOpacity>
        </View>
        ...
        {menuOptions.map((option, i) => {
          ...
          return (
            <View
              key={option.text}
              style={{ borderBottomWidth: last ? 0 : 0.5, borderColor: '#555' }}
            >
              <TouchableOpacity onPress={this._showAlert.bind(this)}>
                <View
                  style={{
                    flexDirection: 'row',
                    paddingVertical: 10,
